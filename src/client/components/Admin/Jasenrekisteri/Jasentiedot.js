...
import { updateSelectedMember, clearSelectedMember, saveMember, approveMember, deleteMember } from '../../../actions/jasenrekisteriActions';

const Jasentiedot = ({
  member, update, clear, save, approve, remove,
}) => {
  const handleDelete = () => {
    if(confirm('Poistetaanko jäsen rekisteristä? Toimintoa ei voi peruuttaa')) {
      remove(member);
    }
  };
  return (
    <div>
      ...
      <div className="row">
        <div className="col">
          <button onClick={handleDelete} className="btn btn-danger">Poista rekisteristä</button>
        </div>
      </div>
    </div>
  );
...
const mapDispatchToProps = dispatch => ({
  ...
  remove: member => dispatch(deleteMember(member)),
});
